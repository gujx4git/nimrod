...
import com.gioov.nimrod.common.config.CommonConfig;
...
public class SimpleUserDetailsServiceImpl implements UserDetailsService {
    ...
    private boolean isExistRole(String roleValue) {
        List<String> roleList = commonConfig.getSystemAdminRole();
        ...
        if (userRoleEntityList != null) {
            ...
            for (UserRoleEntity userRoleEntity : userRoleEntityList) {
                ...
                if (roleEntity != null) {
                    ...
                    simpleGrantedAuthorityList1.add(new SimpleGrantedAuthority(rolePrefix + roleValue));
                    // 读取和装载角色权限
            ...
            for (SimpleGrantedAuthority simpleGrantedAuthority : simpleGrantedAuthorityList1) {
                i++;
                if (!simpleGrantedAuthority.getAuthority().equals(rolePrefix + SYSTEM_ADMIN) && (i == simpleGrantedAuthorityList1.size()) && isExistRole) {
                    simpleGrantedAuthorityList.add(new SimpleGrantedAuthority(rolePrefix + SYSTEM_ADMIN));
                }
            }
        }
